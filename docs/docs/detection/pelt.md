...
- $t_0 = 0 < t_1 < \dots < t_m < t_{m+1} = N$ are the change point indices. $m$ is the total number of change points
  detected.
- $y_{a:b}$ represents the segment of the time series from index $a$ up to (and including) index $b-1$. Note: In the formula $y_{t_i+1 : t_{i+1}}$, the indices refer to the data range $y[t_i], \dots, y[t_{i+1}-1]$ in 0-based indexing, covering a length of $t_{i+1} - t_i$. The `ComputeCost` method uses `start` (inclusive) and `end` (exclusive) indices, so `ComputeCost(t_i, t_{i+1})` would compute the cost for this segment.
...
- **`CostFunction`**: An instance implementing @"SignalSharp.CostFunctions.Cost.IPELTCostFunction?text=IPELTCostFunction". This determines how the cost of each potential segment is
...
- **`MinSize`**: The minimum allowable number of data points within a segment. Ensures that segments are not too small.
  Must be greater than or equal to 1 (cost functions may impose larger minimums, e.g., @"
  SignalSharp.CostFunctions.Cost.ARCostFunction?text=ARCostFunction"). *Default: 1*.
- **`Jump`**: Controls the step size when considering previous potential change points. *Default: 1*.
...
methods like BIC/AIC, elbow plots, or cross-validation if applicable). See the Automatic Penalty Selection section below for methods to help automate this.
...
#### @"SignalSharp.CostFunctions.Cost.L1CostFunction?text=L1"

The L1 cost function measures the sum of absolute deviations from the segment median:

$$ \mathcal{C}(y_{a:b}) = \sum_{i=a}^{b-1} \sum_{d=0}^{D-1} | y_{d,i} - \text{median}(y_{d, a:b}) | $$

where $D$ is the number of dimensions, $y_{d,i}$ is the value at dimension $d$ and time $i$, and $y_{d, a:b}$ represents the segment data for dimension $d$.
...
#### @"SignalSharp.CostFunctions.Cost.L2CostFunction?text=L2"

The L2 cost function measures the sum of squared deviations from the segment mean:

$$ \mathcal{C}(y_{a:b}) = \sum_{d=0}^{D-1} \sum_{i=a}^{b-1} ( y_{d,i} - \text{mean}(y_{d, a:b}) )^2 $$

This is equivalent to minimizing the within-segment sum of squares (variance scaled by length), assuming variance is constant across segments. The L2
...
#### @"SignalSharp.CostFunctions.Cost.GaussianLikelihoodCostFunction?text=Gaussian%20Likelihood"

The Gaussian likelihood cost function measures the Gaussian negative log-likelihood. The cost/metric is primarily driven by the
term:

$$ \mathcal{C}(y_{a:b}) \approx \sum_{d=0}^{D-1} (b-a) \times \log(\hat{\sigma}^2_{d, a:b}) $$

where $\hat{\sigma}^2_{d, a:b}$ is the Maximum Likelihood Estimate (MLE) of the variance for the segment $y_{a:b}$ in dimension $d$.
Constant terms related to $(b-a)$ and $\pi$ are often omitted as they don't affect the location of change points when
using a penalty or information criteria.
...
data within segments follows a Gaussian distribution (independently per dimension).

The implementation is similar to L2, using prefix sums of data and squared data (O(N * D) precomputation). Segment cost/metric
calculation is O(D). It includes a small epsilon floor for variance to handle segments with zero variance numerically.
...
segment size of at least 1. It implements `ILikelihoodCostFunction` and supports BIC/AIC/AICc penalty selection.

#### @"SignalSharp.CostFunctions.Cost.PoissonLikelihoodCostFunction?text=Poisson%20Likelihood"

The Poisson likelihood cost function is based on the Poisson negative log-likelihood, suitable for count data. The cost/metric
is derived from:

$$ \mathcal{C}(y_{a:b}) = \sum_{d=0}^{D-1} 2 \left[ S_d - S_d \log(S_d) + S_d \log(n) \right] $$

where $S_d = \sum_{i=a}^{b-1} y_{d,i}$ is the sum of counts in the segment for dimension $d$, $n = b-a$ is the segment length, and
it assumes $0 \log(0) = 0$. Constant terms like $\sum \log(y!)$ are omitted.

This function detects changes in the underlying rate ($\lambda_d$) of events for each dimension. It assumes that counts within a segment
follow a Poisson distribution with a constant rate, estimated by the sample mean $\hat{\lambda}_d = S_d/n$. It is sensitive
to changes in the average count level.

The implementation uses prefix sums of the data (O(N * D) precomputation). Segment cost/metric calculation is O(D). Input data
must be non-negative counts (small negative values close to zero might be tolerated by clamping to 0).
...
multidimensional data and requires a minimum segment size of at least 1. It implements `ILikelihoodCostFunction` and supports BIC/AIC/AICc penalty selection.

#### @"SignalSharp.CostFunctions.Cost.BernoulliLikelihoodCostFunction?text=Bernoulli%20Likelihood"

The Bernoulli likelihood cost function is derived from the Binomial negative log-likelihood for binary (0/1) data. The
cost/metric is:

$$ \mathcal{C}(y_{a:b}) = \sum_{d=0}^{D-1} -2 \left[ S_d \log(S_d) + (n-S_d) \log(n-S_d) - n \log(n) \right] $$

where $S_d = \sum_{i=a}^{b-1} y_{d,i}$ is the number of successes (sum of 1s) in dimension $d$, $n = b-a$ is the segment length, and $n-S_d$ is
the number of failures (sum of 0s). It assumes $0 \log(0) = 0$. Combinatorial terms are omitted.

This function detects changes in the probability of success ($p_d$) in a sequence of Bernoulli trials for each dimension. It assumes data
within a segment consists of independent 0/1 outcomes with a constant success probability, estimated by the sample
proportion $\hat{p}_d = S_d/n$.
...
#### @"SignalSharp.CostFunctions.Cost.BinomialLikelihoodCostFunction?text=Binomial%20Likelihood"

The Binomial likelihood cost function is used for data where each time point represents $k$ successes out of $n$ trials.
The cost/metric function is:
...
trials in the segment. It assumes $0 \log(0) = 0$. Combinatorial terms are omitted.
...
trials ($n_i$). It uses prefix sums for both $k$ and $n$ (O(N) precomputation, as D=2 is fixed). Segment cost/metric calculation is O(
1). Input $k_i$ and $n_i$ must be non-negative integers (within tolerance) with $0 \le k_i \le n_i$ and $n_i \ge 1$.
...
least 1. It implements `ILikelihoodCostFunction` and supports BIC/AIC/AICc penalty selection.

#### @"SignalSharp.CostFunctions.Cost.RBFCostFunction?text=RBF"
...
$$ \mathcal{C}(y_{a:b}) = \sum_{d=0}^{D-1} \left( (b-a) - \frac{1}{b-a} \sum_{i=a}^{b-1} \sum_{j=a}^{b-1} \mathcal{K}(y_{d,i}, y_{d,j}) \right) $$
...
texture. It supports multidimensional data and requires a minimum segment size of at least 1. It does not support likelihood-based penalty selection.

#### @"SignalSharp.CostFunctions.Cost.ARCostFunction?text=Autoregressive"
...
`MinSize >= max(order + 1, 2 * order + k)`, where `k=1` if an intercept is included, `k=0` otherwise. It implements `ILikelihoodCostFunction` (calculating likelihood based on RSS and Gaussian error assumptions) and supports BIC/AIC/AICc penalty selection.
...
**Need for Automatic Penalty Selection**: If you plan to use BIC, AIC, or AICc for automatic penalty selection (see below), you must choose a cost function that implements `ILikelihoodCostFunction` and sets `SupportsInformationCriteria` to true (Gaussian, Poisson, Bernoulli, Binomial, AR).
...
| **Segment Cost/Metric**  | O(seg_len*D) (approx)           | O(D) (very fast)                 | O(D) (very fast)                | O(D) (very fast)          | O(D) (very fast)          | O(1) (very fast)               | O(D) (fast, after precomp)                  | ~O(seg_len*order²) (relatively slow)       |
...
| **Supports BIC/AIC/AICc** | No                              | No                               | Yes                             | Yes                       | Yes                       | Yes                            | No                                          | Yes                                        |
...
*   **For financial or environmental data** where both the level and volatility matter, use **Gaussian-Likelihood**. It captures those periods where not just the average changes, but also how wildly the values fluctuate – perfect for detecting market regime shifts or climate pattern changes. It also supports automatic penalty selection.
*   **Working with time series that show patterns or cycles?** If you're analyzing a *univariate* series where the relationship between consecutive values matters more than their absolute levels, **AR** is your best bet. Remember to choose an order that matches your data's memory. It also supports automatic penalty selection.
...
**Need automatic penalty selection?** If you plan to use BIC, AIC, or AICc, ensure you pick **Gaussian-Likelihood**, **Poisson-Likelihood**, **Bernoulli-Likelihood**, **Binomial-Likelihood**, or **AR**.
...
sensitivity. For likelihood-based costs, consider using automatic penalty selection.)*
...
// Configure PELT (using default L2 cost, MinSize=1, Jump=1)
var options = new PELTOptions(); // Defaults: L2CostFunction, MinSize=1, Jump=1
...
var options = new PELTOptions
{
    ...
    MinSize = 2, // L1 often needs slightly larger min size for stable median
...
// Example result (depends heavily on penalty): [3, 7] or similar
```
...
var options = new PELTOptions
{
    ...
    MinSize = 3 // Ensure enough points for variance estimation
};
var pelt = new PELTAlgorithm(options);
...
// Example result (depends on gamma and penalty): Possibly around index 3 or 7
```
...
// Helper function to generate AR(1) data (replace with actual generation if needed)
/*
public static double[] GenerateAR1Data(int n, double phi, double c, double sigma, int seed=42)
{
    var random = new Random(seed);
    var data = new double[n];
    data[0] = c / (1 - phi); // Start near expected mean
    for (int t = 1; t < n; t++)
    {
        data[t] = c + phi * data[t - 1] + random.NextGaussian(0, sigma); // Assuming NextGaussian exists
    }
    return data;
}
*/
...
// Example placeholder using simpler manual data
double[] signalPart1 = { 1.0, 0.9, 0.85, 0.82, 0.81 }; // Segment 1: High positive correlation
double[] signalPart2 = { 0.2, -0.1, 0.05, -0.02, 0.01 }; // Segment 2: Lower/negative correlation
...
var options = new PELTOptions
{
    ...
    MinSize = 3, // AR(1)+intercept requires min length 3 (check ARCostFunction docs)
    Jump = 1 // Use exact for potentially better results
};
var pelt = new PELTAlgorithm(options);

int[] changePoints = pelt.FitAndDetect(arSignal, penalty: 0.05); // Penalty needs tuning based on RSS scale
...
var options = new PELTOptions { CostFunction = new L1CostFunction(), MinSize = 2 };
var pelt = new PELTAlgorithm(options);
pelt.Fit(sensorData);
...
Choosing the right penalty ($\beta$) is often challenging. SignalSharp provides a @"SignalSharp.Detection.PELT.PELTPenaltySelector" to help automate this process using information criteria like BIC, AIC, or AICc.

### How it Works

The penalty selector tests a range of penalty values. For each penalty, it runs the PELT algorithm to find the corresponding change points. Then, it calculates a score based on the chosen information criterion (e.g., BIC). The penalty value that results in the optimal (usually minimum) score is selected.

-   **BIC (Bayesian Information Criterion)**: Tends to favor simpler models (fewer change points). Formula: `-2*logL + P*log(N)`
-   **AIC (Akaike Information Criterion)**: May select slightly more complex models than BIC. Formula: `-2*logL + 2*P`
-   **AICc (Corrected AIC)**: AIC with a correction for small sample sizes. Recommended when N is not much larger than P. Formula: `AIC + (2*P*(P+1))/(N-P-1)`

Where:
-   `logL` is the maximized log-likelihood of the data given the segmentation.
-   `P` is the total number of effective parameters in the model (sum of parameters within each segment + number of change points).
-   `N` is the total number of data points.

### Requirements

Automatic penalty selection using BIC, AIC, or AICc **requires** the PELT algorithm to be configured with a cost function that implements @"SignalSharp.CostFunctions.Cost.ILikelihoodCostFunction?text=ILikelihoodCostFunction" and has `SupportsInformationCriteria = true`. Suitable cost functions include:
...
### Configuration (@"SignalSharp.Detection.PELT.PELTPenaltySelectionOptions?text=PELTPenaltySelectionOptions")

The selection process is configured using @"SignalSharp.Detection.PELT.PELTPenaltySelectionOptions":

-   **`Method`**: The criterion to use (@"SignalSharp.Detection.PELT.PELTPenaltySelectionMethod.BIC?text=BIC", @"SignalSharp.Detection.PELT.PELTPenaltySelectionMethod.AIC?text=AIC", @"SignalSharp.Detection.PELT.PELTPenaltySelectionMethod.AICc?text=AICc"). *Required*.
-   **`MinPenalty`**: The minimum penalty value to test. *Optional (heuristic used if null)*.
-   **`MaxPenalty`**: The maximum penalty value to test. *Optional (heuristic used if null)*.
-   **`NumPenaltySteps`**: Approximate number of penalties to test between Min and Max (logarithmically spaced). *Default: 50*.

### Usage Example: Automatic Penalty Selection with BIC
...
// Sample signal where Gaussian Likelihood is appropriate
double[] signal = { 0, 0.1, -0.1, 0, 0.1, 3.0, -2.0, 1.0, -3.0, 2.5 };

// 1. Configure PELT with a likelihood-based cost function
var peltOptions = new PELTOptions
{
    CostFunction = new GaussianLikelihoodCostFunction(),
    MinSize = 3 // Gaussian needs enough points for mean/variance
};
var peltAlgorithm = new PELTAlgorithm(peltOptions);

// 2. Create the Penalty Selector
var selector = new PELTPenaltySelector(peltAlgorithm);

// 3. Configure the selection options (e.g., use BIC)
var selectionOptions = new PELTPenaltySelectionOptions(PELTPenaltySelectionMethod.BIC)
{
    // Optionally specify Min/Max penalty or steps, otherwise defaults/heuristics are used
    // MinPenalty = 0.5,
    // MaxPenalty = 10.0,
    // NumPenaltySteps = 100
};

// 4. Fit the data and select the penalty (this also fits the internal PELT algorithm)
PELTPenaltySelectionResult result = selector.FitAndSelect(signal, selectionOptions);

// 5. Use the results
Console.WriteLine($"Selected Penalty (BIC): {result.SelectedPenalty:F4}");
Console.WriteLine("Optimal Change Points: " + string.Join(", ", result.OptimalBreakpoints));
// Expected: Around index 5, penalty value will vary based on data and BIC calculation.

// Access diagnostics if needed (list of (Penalty, Score, NumChangePoints))
if (result.Diagnostics != null)
{
    Console.WriteLine("\nDiagnostics:");
    foreach (var diag in result.Diagnostics)
    {
        Console.WriteLine($"  Penalty: {diag.Penalty:F4}, Score: {diag.Score:F4}, CPs: {diag.ChangePoints}");
    }
}

// Alternatively, if PELT is already fitted:
// peltAlgorithm.Fit(signal); // Ensure it's fitted
// PELTPenaltySelectionResult result = selector.SelectPenalty(selectionOptions); // Select penalty on pre-fitted algo (Internal method in example, use FitAndSelect generally)
```
...
1.  **Computational Efficiency for Large Datasets**:
    *   PELT employs a pruning strategy that significantly reduces computational complexity, often achieving near-linear
      time performance (O(N)) relative to the time series length (N), especially with optimized cost functions like L2 or likelihood-based ones using prefix sums.
...
4.  **Simultaneous Analysis of Multiple Time Series (Multidimensional Support)**:
    *   PELT can process multivariate time series (provided as `double[,]` where rows represent dimensions/channels and
      columns represent time points), provided the chosen cost function supports it. It identifies common breakpoints where the statistical properties change across
      *all* dimensions considered together, providing insights into systemic shifts. (Note: `ARCostFunction` is currently univariate only).

5.  **Support for Automatic Penalty Selection**:
    *   Using `PELTPenaltySelector` with likelihood-based cost functions allows for automated, data-driven selection of the penalty parameter using established information criteria (BIC, AIC, AICc).
...
1.  **Sensitivity to Parameter Selection (Penalty and `MinSize`)**:
    *   The quality of the segmentation is highly dependent on the chosen `penalty` value. This parameter directly
      ...
      points). Selecting an optimal penalty often requires experimentation, domain expertise, or the use of methods like the `PELTPenaltySelector`. An inappropriate penalty can lead to significant under- or
      over-segmentation.
    *   The `MinSize` parameter, defining the minimum segment length, also influences results. It prevents trivial
      segmentations but can mask short-lived events if set too high. Some cost functions (like AR) have specific minimum size requirements.
...
6.  **Cost Function Computational Cost**:
    *   Overall runtime depends on PELT's pruning and the segment cost calculation:
        *   O(1) or O(D) segment cost/metric (fast): L2, Gaussian, Poisson, Bernoulli, Binomial, RBF (after precomputation)
        *   Slower segment cost/metric: L1 (O(seg_len*D)), AR (O(seg_len*order²))
        *   Slow precomputation: L1 (O(N²*D*logN)), RBF (O(N²*D))

7.  **Penalty Selection Requirements**:
    *   Automatic penalty selection via BIC/AIC/AICc requires specific likelihood-based cost functions. Other cost functions (L1, L2, RBF) require manual penalty tuning.
...
-   **Cost Functions**:
    ...
    -   @"SignalSharp.CostFunctions.Cost.ILikelihoodCostFunction" (Optional interface for BIC/AIC support)
...
-   **Automatic Penalty Selection**:
    -   @"SignalSharp.Detection.PELT.PELTPenaltySelector"
    -   @"SignalSharp.Detection.PELT.PELTPenaltySelectionOptions"
    -   @"SignalSharp.Detection.PELT.PELTPenaltySelectionResult"
    -   @"SignalSharp.Detection.PELT.PELTPenaltySelectionMethod"