...
public class CUSUMAlgorithmTests
{
    ...
    public void Detect_SimpleSignal_ShouldDetectChangePoints()
    {
        ...
        var signal = StatisticalFunctions.ZScoreNormalization([0.2, 1.1, 0.2, 4.0, 0.1, 0.2, -2.0, 0.2, 0.1]).ToArray();
    ...
    public void Detect_NoChangeSignal_ShouldDetectNoChangePoints()
    {
        ...
        var cusum = new CUSUMAlgorithm(options);
        var signal = StatisticalFunctions.ZScoreNormalization([0.2, 0.1, 0.2, 0.1, 0.2, 0.1, 0.2, 0.1]).ToArray();
    ...
    public void Detect_GradualChangeSignal_ShouldDetectChangePoints()
    {
        ...
        var cusum = new CUSUMAlgorithm(options);
        var signal = StatisticalFunctions.ZScoreNormalization<double>(Enumerable.Range(0, 20).Select(i => i == 9 ? 15.0 : i * 0.1).ToArray()).ToArray();
    ...
    public void Detect_SuddenSpikeSignal_ShouldDetectChangePoints()
    {
        ...
        var signal = StatisticalFunctions.ZScoreNormalization([0.1, 0.2, 0.3, 10.0, 0.1, 0.2, -10.0, 0.1, 0.2]).ToArray();
    ...
    public void Detect_LongConstantSignal_ShouldDetectNoChangePoints()
    {
        ...
        var cusum = new CUSUMAlgorithm(options);
        var signal = StatisticalFunctions.ZScoreNormalization<double>(Enumerable.Repeat(0.1, 100).ToArray()).ToArray();
        int[] expectedChangePoints = [];
    ...
    {
        var options = new CUSUMOptions { ExpectedStandardDeviation = 0 };

        Assert.Throws<ArgumentOutOfRangeException>(() =>
        {
            _ = new CUSUMAlgorithm(options);
        });
    }
    ...
    {
        var options = new CUSUMOptions { ThresholdFactor = 0 };
    ...
    {
        var options = new CUSUMOptions { SlackFactor = -1 };
